    ...
    return (
        <div id="add-product" className="page-height page-padding">
            ...
            <form id="add-product-form" name="login" method="POST" encType="multipart/form-data">
                ...
                <Webcam audio={false} screenshotFormat="image/png" screenshotQuality={1} videoConstraints={{ width: 600, height: 600 }} height={600} width={600} resizeMode='contain' ref={webcam} />
                {/* 720x720 */}
                <br></br>
                <button onClick={(e) => {
                    e.preventDefault();

                    setFrontImageBase64(webcam.current.getScreenshot({ width: 600, height: 600 }));
                    setFrontImageBlob(base64toBlob(webcam.current.getScreenshot({ width: 600, height: 600 }).split(",")[1], "image/png"));

                }}>Take Front Image</button>
                <button onClick={(e) => {
                    e.preventDefault();

                    setBackImageBase64(webcam.current.getScreenshot({ width: 600, height: 600 }));
                    setBackImageBlob(base64toBlob(webcam.current.getScreenshot({ width: 600, height: 600 }).split(",")[1], "image/png"));
